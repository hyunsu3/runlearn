...
function applyTouchEvents() {
  ...
  const audioEng = new Audio();
  const audioKor = new Audio();
  const wrongAudios = ["Audio/wrong1.mp3", "Audio/wrong2.mp3"];
  ...
  draggables.forEach((draggable) => {
    ...
    function onEnd() {
      ...
      if (!droppedCorrectly) {
        ...
        // 오답 음성 랜덤 재생
        const wrongAudio = new Audio(wrongAudios[Math.floor(Math.random() * wrongAudios.length)]);
        wrongAudio.play().catch(console.error);
      }
